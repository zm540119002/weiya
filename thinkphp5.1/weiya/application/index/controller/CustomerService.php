            ...
            if($this->user){//发送者为：注册用户
                ...
                if($postData['to_user_id']){//接收者为：注册用户
                    ...
                    if(Gateway::isOnline($postData['to_client_id'])){//在线
                        ...
                        return $this->errorMsg('对方未在线！');
                    }
                }else{
                    return $this->errorMsg('缺少参数');
                }
            }else{//发送者为：游客
                $msg = [
                    'type' => 'msg',
    ...
    public function setMessageRead(){
        if(request()->isAjax()){
            ...
            if($this->user){
                ...
                if($res===false){
                    return $this->errorMsg('设置已读出错',$modelChatMessage->getError());
                }
                return successMsg('成功！');
            }
        }
    ...
    public function delCustomerMessage(){
        if(request()->isAjax()){
            ...
            if($this->user){
                ...
                if($res===false){
                    return $this->errorMsg('删除失败！',$modelChatMessage->getError());
                }
